...
import { TEXT_SOURCES, OUTPUT_LANGUAGES, TONE_OF_VOICE, IMAGE_OPTIONS, LOGO_OPTIONS, STYLES } from '../../data/options'
...
export default function SectionContentDesign({ form, onChange }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <FormField label="Jazyk webu">
        <select value={form.outputLanguage} onChange={(e) => onChange('outputLanguage', e.target.value)} className="input">
          <option value="">Vyberte...</option>
          {OUTPUT_LANGUAGES.map((l) => <option key={l}>{l}</option>)}
        </select>
      </FormField>
      ...
      <FormField label="Tón komunikace">
        <MultiSelectButtons options={TONE_OF_VOICE} value={form.toneOfVoice} onChange={(v) => onChange('toneOfVoice', v)} />
      </FormField>
